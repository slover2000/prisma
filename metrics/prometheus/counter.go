...
func (c *prometheusClient) CounterGRPC(name string, duration time.Duration, err error) {
	...
	// 记录Counter, like QPS
	c.qpsCounter.WithLabelValues(serviceName, methodName, code.String()).Inc()
	...
	c.latencyHistogram.WithLabelValues(serviceName, methodName, code.String()).Observe(float64(ms))
}
...
func (c *prometheusClient) CounterHTTP(req *http.Request, duration time.Duration, code int) {
	...
	// 记录Counter, like QPS
	c.qpsCounter.WithLabelValues(req.URL.Host, req.URL.Path, req.Method, fmt.Sprintf("%d", code)).Inc()
	...
	c.latencyHistogram.WithLabelValues(req.URL.Host, req.URL.Path, req.Method, fmt.Sprintf("%d", code)).Observe(float64(ms))
}
